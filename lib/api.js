...
let API = function(app, express, basePath){
    let handlers = [];
    let self=this;
                    ...
                    if(stat.isFile() && fnme.match(rx)){
                        ...
                        handlerDefs.forEach(
                            def => {
                                if(def.rest){
                                    let rest = scope.exec(
                                            function(api, def, store){
                                                return api.createRest( def.name, store[def.store], def.filters );
                                            },
                                            {api: this, def: def.rest}
                                        );
                                        if(def.rest.onbuilt){
                                            def.rest.onbuilt( rest );
                                        }
                                } else {
                                    self.addHandler( def );
                                }
