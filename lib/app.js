...
let log = require('nodeutils').log.create('expressd:app');
...
function APP(options){
    ...
    let loadModules = ()=>{
        ...
        if(opts.moduleDir){
            opts.moduleDir.split(',').forEach(
                mdir => {
                    mloader.modulesFromDir( getdir( mdir.trim() ) );
                }
            );
        } else {
            mloader.modulesFromDir( getdir( 'modules' ) );
        }
        mloader.modulesFromDir( path.join( __dirname, '../modules' ) );
    ...
    this.start = ()=>{
        ...
        let sessconfig = opts.express.session||{};
